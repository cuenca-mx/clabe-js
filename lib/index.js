...
function generateNewClabes(numberOfClabes, prefix) {
  ...
  */
  ...
  const lowerBound = Math.pow(10, missing - 1);
  const upperBound = Math.pow(10, missing);
  const range = upperBound - lowerBound;
  if (range < numberOfClabes) {
  ...
  while (clabes.length < numberOfClabes) {
    const section = lowerBound + Math.floor(Math.random() * range);
    if (seen.has(section)) continue;
    seen.add(section);
    const partial = prefix + String(section);
...
function removeBank(bankCodeBanxico) {
  ...
  */
  ...
  delete BANKS[bankCodeBanxico.slice(-3)];
